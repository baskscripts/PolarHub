...
-- Configuration
local espRadius = 1000000000  -- Radius in studs to highlight players
...
local function createESP(player)
    ...
    if playerCharacter then
        ...
        if torso then
            ...
            -- Store the ESP part for future reference
            espParts[player.UserId] = espPart
        end
    end
end
...
local function updateESP()
    for _, otherPlayer in ipairs(game.Players:GetPlayers()) do
        if otherPlayer ~= player then
            if not espParts[otherPlayer.UserId] then
...
-- Handle initial ESP creation for all players
for _, otherPlayer in ipairs(game.Players:GetPlayers()) do
    if otherPlayer ~= player then
        createESP(otherPlayer)
    end
